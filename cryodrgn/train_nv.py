...
from cryodrgn.sync_batchnorm import SynchronizedBatchNorm1d, patch_replication_callback
...
def train_batch(model, lattice, y, yt, rot, trans, optim, beta,
    ...
    mask = 70
    z_mu, z_logstd, z, y_recon, y_recon_tilt, losses, y, y_ffts, mus, top_mus = run_batch(model, lattice, y, yt, rot,
    ...
    if group_stat is not None:
        mask_sum = group_stat.get_mask_sum(mask)*2.
    ...
    loss, gen_loss, kld, mu2, std2, mmd, c_mmd = loss_function(z_mu, z_logstd, y, yt, y_recon, mask,
                                        ...
                                        losses=losses, args=args, it=it, y_ffts=y_ffts, zs=z, mus=mus, top_mus=top_mus)
    if use_amp:
...
def sample_neighbors(posetracker, data, euler, ind, ctf_params, ctf_grid, mask, grid, args, W, out_size):
...
def run_batch(model, lattice, y, yt, rot, tilt=None, ind=None, ctf_params=None,
    ...
    return z_mu, z_logstd, z, y_recon, y_recon_tilt, losses, y_ref, y_ffts, mus, top_mus
...
def loss_function(z_mu, z_logstd, y, yt, y_recon, mask, beta,
    ...
    if not vanilla:
        ...
        #print(em_l2_loss.shape, z_mu.shape, probs)
        ...
        em_l2_loss = torch.sum(em_l2_loss)/(mask_sum*B*C)
...
def main(args):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print('learning rate {} for epoch {}'.format(lr_scheduler.get_last_lr(), epoch))
        for minibatch in data_generator:
